Attribute VB_Name = "mFontGenerator"
Option Explicit
...
'
' Also see:
' http://www.planetsourcecode.com/vb/scripts/ShowCode.asp?txtCodeId=50004&lngWId=1
' ================================================================================
...
Public Function DoCreateFonts(objForm As Form) As VBOpenGL.GLuint

    objForm.Font = "Arial Black"
    objForm.FontSize = 10
    objForm.FontBold = False
    
    Dim listID As VBOpenGL.GLuint
    listID = VBOpenGL.glGenLists(24)
    If listID = 0 Then Call Err.Raise(Err.LastDllError, "mFontGenerator.DoCreateFonts", "Could not create a new list identifer.")
    
    DoCreateFonts = listID
    
    ' 24 characters A-Z
    Dim intN As Integer
    For intN = 33 To 126
    
        Call VBOpenGL.glNewList(listID + (intN - 33), lstCompile)
        
            Call mFontGenerator.GetGlyphs(objForm.hdc, 0.5, intN)
            Call DrawGlyph
            
            ' Draw a space.
            Call VBOpenGL.glTranslated(lpgm.gmCellIncX, 0, 0)
            
        Call VBOpenGL.glEndList
...
Public Sub DrawGlyph()
    ...
    For intPoly = LBound(m_objPolygons) To UBound(m_objPolygons)
        ...
        Call VBOpenGL.glBegin(bmLineLoop)
        For intVertex = LBound(m_objPolygons(intPoly).Vertex) To UBound(m_objPolygons(intPoly).Vertex)
            ...
            Call VBOpenGL.glVertex3f(sngX, sngY, 0)
